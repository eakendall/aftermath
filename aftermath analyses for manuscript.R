...
# no longer assuming resouce equivalency, so will estimate cost separately
cohort_param_ranges <- list(
  ...
  intentional_counseling_passive_detection_impact = c(0.8), # not using
  intentional_counseling_passive_detection_duration = 180, # days before effect of counseling on care seeking "wears off" # not using
  prevention_efficacy = 0.6, # not using
  ...
  prevention_cost = c(7.24, 9.14), # not using
...
saveRDS(cohort_params, file="cohort_params_aftermath_20260126.rds")
saveRDS(cohort_features, file="cohort_features_aftermath_20260126.rds")    
saveRDS(cascade_features, file="cascade_features_aftermath_20260126.rds")    
# } else {
  cohort_params <- readRDS(file="cohort_params_aftermath_20260126.rds")    
  cohort_features <- readRDS(file="cohort_features_aftermath_20260126.rds")
  cascade_features <- readRDS(file="cascade_features_aftermath_20260126.rds")
}
  ...
  for (n in 1:N_samples)
  {
    ...
    for (intervention in intervention_names)
    {
      outputs[[intervention]] <- apply_intervention(cohort, get(paste0("screening_design_", intervention)), 
                                                    intervention_parameters, cohort_params[n,])  
      cost[[intervention]] <- costs(get(paste0("screening_design_", intervention)), cohort, cohort_params[n,]) 
    }
...
param_names <- data.frame(
  ...
  # 1] "N"                                                 "recurrence_time_mean"                             
  # [3] "recurrence_time_shapefactor"                       "incidence_18mo"                                   
  # [5] "proportion_micro_pos"                              "auc"                                              
  # [7] "symptom_duration_mean_reported"                    "symptom_duration_sd_reported"                     
  # [9] "symptom_duration_timescale"                        "symptom_underestimation_factor"                   
  # [11] "proportion_ever_subclinical"                       "duration_ratio_subclinical_symptomatic"           
  # [13] "duration_subclinical_cv"                           "subclinical_baseline_amongTB_max"                 
  # [15] "subclinical_6m_amongcohort_min"                    "subclinical_6m_amongcohort_max"                   
  # [17] "coverage_phone"                                    "coverage_home_reduction"                          
  # [19] "sensitivity_symptoms_home"                         "sensitivity_symptoms_phone_reduction"             
  # [21] "success_sputum_home"                               "success_sputum_phone_reduction"                   
  # [23] "home_visit_passive_detection_impact"               
  ...
  # [31] "prevention_cost"                                  
  name = c("cohort size", 
           "Shape factor, time to TB recurrence",
           "Scale factor, time to TB recurrence",
           "Mean time to TB recurrence",
           "Variance, time to TB recurrence",
           "Cumulative 18-month symptomatic incidence",
           "Proportion bacteriologically+ at diagnosis",
           "Predictive accuracy for recurrence (AUC)",
           "Mean duration of reported symptoms",
           "SD of duration of reported symptoms",
           "Change in symptom duration over time (timescale)",
           ...
           "Duration of effect of care-seeking-promotion intervention",
           "Effectiveness of prevention intervention (reduction in recurrence risk)",
